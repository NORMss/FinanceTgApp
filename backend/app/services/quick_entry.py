...
чем открыть Mini App. Формат намеренно свободный:
    ...
    1 200,50 такси домой    -> расход 1200.50, «Транспорт · Такси», заметка «домой»
    600 пятёрочка молоко    -> расход 600, «Продукты · Пятёрочка», заметка «молоко»
...
потом правила автокатегоризации по подстроке. Подкатегории участвуют в обоих на равных —
попасть сразу в «Пятёрочку» удобнее, чем потом уточнять её руками.
"""
