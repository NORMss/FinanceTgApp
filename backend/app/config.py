...
class Settings(BaseSettings):
    ...
    # --- Напоминания ---
    # Общий выключатель: с ним не нужно обходить всех пользователей, чтобы прекратить рассылку
    reminders_enabled: bool = True
    # Пояс для тех, чей клиент его ещё не сообщил (например, человек пока заходил только
    # в бота). Mini App при входе присылает свою зону, и она перекрывает эту.
