...
Как вести общий бюджет, откуда берётся «кто кому должен» и зачем нужен общий счёт:
...
| Правка записи | Справочник категорий | Напоминание и расчёты |
|:---:|:---:|:---:|
| ![Шторка правки операции](docs/screenshots/edit.png) | ![Дерево категорий с эмодзи](docs/screenshots/categories.png) | ![Время напоминания, остатки и взаиморасчёты](docs/screenshots/more.png) |
| Любое поле уже записанной операции | Создать, переименовать, сменить значок | Своё время напоминания, кто кому должен |
...
- **Фильтры**: история и отчёт разбираются по людям, типу операции и категории;
  выбранная категория тянет за собой свои подкатегории. Фильтр по людям отвечает
  на вопрос «чья трата», а не «кто её записал».
...
- **Напоминания**: вечером бот пишет тому, кто за день ничего не записал. Время каждый
  задаёт себе сам в приложении, считается оно по его часовому поясу.
- **Счета**: по умолчанию трата уходит на личный счёт автора. Общий счёт заводится
  отдельно и делит траты поровну — приложение считает, кто кому должен, и учитывает
  переводы-погашения. Разбор модели — в [docs/finance.md](docs/finance.md).
...
| Фронт | React 18 + TypeScript + Vite, TanStack Query, ~69 КБ gzip |
...
- Управление — вкладка **Ещё → Категории**: создать, переименовать, сменить значок,
  добавить подкатегорию, спрятать, удалить.
...
- **Удаление требует замены.** Приложение сначала показывает, сколько операций висит
  на категории и её подкатегориях, и просит выбрать, куда их перенести — существующую
  категорию или новую, тут же созданную. Без замены удаляется только то, на чём ничего
  нет. Операции не могут остаться без категории: отчёт за прошлый месяц перестал бы
  сходиться, а вернуть разбивку было бы неоткуда.
- **Спрятать — не то же самое, что удалить.** Скрытая категория исчезает из списков
  выбора, но история остаётся как была. Удаление переписывает историю: операции
  переезжают в категорию-замену. Когда прошлое важно — прячьте. Скрытые собраны
  внизу экрана и возвращаются одной кнопкой.
- Вместе с категорией удаляются её подкатегории, а правила быстрого ввода
  (`пятёроч → …`) переезжают в замену, чтобы ввод из чата не онемел.
...
<img src="docs/screenshots/category-delete.png" alt="Удаление категории: сколько операций затронуто и куда их перенести" width="320">

Перед удалением приложение показывает цену вопроса: сколько операций висит на категории,
сколько подкатегорий уйдёт вместе с ней и куда всё это переедет.

## Счета и взаиморасчёты

- Новая запись по умолчанию уходит на **личный счёт автора** — и в приложении, и в боте.
- **Записать за другого** — выбрать в списке его личный счёт. Трата будет числиться
  за ним: в его истории и в его колонке отчёта, хотя вносили её вы. Долга это
  не создаёт — делит только общий счёт.
- **Общего счёта на свежей установке нет.** Он заводится кнопкой в **Ещё →
  Взаиморасчёты**, когда общий кошелёк действительно есть. Общий счёт ровно один.
- Трата с общего счёта делится поровну между участниками и превращается в долг:
  `итог = заплатил − своя доля + погашения`. Долг гасится переводом
  **личный → личный**.
- Доходы не делятся никогда — они меняют остаток, но не долг.
- Смена счёта у записи в истории пересчитывает доли: перенесли трату с общего счёта
  на личный — долг по ней исчез.

Если цифра долга непонятна, попросите показать, из чего она сложилась:

```bash
make check-settle
```

Команда печатает каждую операцию, которая участвует в расчёте, со счётом и долями.
Строки с пометкой `ПРОБЛЕМА` — доли, которых по нынешним правилам быть не может; они
остались у тех, кто правил счёт в версии до 17.08.2026, когда доли пересчитывались
при смене суммы и типа, но не счёта. Пересчитать их разом: `make fix-settle`.

Подробно, с примером на числах и разбором двух моделей общего бюджета —
[docs/finance.md](docs/finance.md).

## Напоминания

Учёт разваливается не от нехватки функций, а от одного забытого вечера. Поэтому бот
вечером пишет тому, кто за день ничего не записал.

- Время — **Ещё → Напоминание**, у каждого участника своё, там же переключатель.
- Считается по часовому поясу человека, а не сервера: Mini App присылает зону браузера
  (`Europe/Moscow`) при каждом входе, так что после переезда или перелёта напоминание
  придёт в новые девять вечера. До первого входа берётся `DEFAULT_TIMEZONE`.
- Записал сам — напоминание не придёт. Проверяются операции, **датированные этим днём**:
  вчерашний чек, добавленный сегодня, сегодняшний день не закрывает.
- Не чаще одного раза в местные сутки. Если сервер в это время перезапускался,
  напоминание уйдёт следующей минутой, а не потеряется.
- Заблокировали бота — повторов не будет: это видно по ответу Telegram.
- Выключить всем сразу, не трогая настройки: `REMINDERS_ENABLED=false`.
...
150 тестов: арифметика денег, проверка `initData`, инварианты журнала (сплиты, балансы,
...
сохранность данных при миграции, наполнение демо, часовые пояса напоминаний, перенос
операций при удалении категории, умолчания по счетам, разбор и починка взаиморасчётов
и отдельный файл на то, что приложение не рассказывает о себе лишнего.
